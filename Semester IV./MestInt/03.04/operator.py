...
class Operator:
    ...
    def precondition_fulfilled(self, hanoi):
        ...
        implication_second_part = min(hanoi.towers[self.from_], default=0) < min(
            hanoi.towers[self.to], default=math.inf
        )
    ...
    def use(self, hanoi):
        ...
        index_untouched = next(
            filter(lambda i: i not in (self.from_, self.to), hanoi.TOWERS)
        )
